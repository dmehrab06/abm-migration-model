...
DIS_EXPONENT = float(sys.argv[3]) #1.5/2.0 ## kind of tuned
A = float(sys.argv[4]) #70.0 ## calibrated
T = float(sys.argv[5]) #0.8/1.0 ## calibrated
S = float(sys.argv[6]) #98.67 ## from paper

#time_choice = random.sample([[4,1],[7,1],[7,3],[12,3],[12,5],[12,8],[14,1],[14,5],[14,7],[14,12],[21,1],[21,3],[21,6],[21,7],[21,10],[21,14],[21,18]],1)[0]
lookbefore_days_left = int(sys.argv[7])
lookbefore_days_right = int(sys.argv[8])

PROB_SCALAR = float(sys.argv[9]) ##make this a parameter
...
THRESH_HI = int(sys.argv[12])

USE_NEIGHBOR = int(sys.argv[13])

BORDER_CROSS_PROB = float(sys.argv[14])
...
#     home_conflict_df['another_random'] = np.random.random(home_conflict_df.shape[0])
#     home_conflict_df['move_type'] = home_conflict_df.apply(lambda x: bernoulli_border(x['another_random'],x['moves']),axis=1)
    
#     new_row = {'id':PLACE_NAME,'time':min_date,'refugee':home_conflict_df[home_conflict_df.move_type==2]['hh_size'].sum(),
#                'old_people':home_conflict_df[home_conflict_df.move_type==2]['OLD_PERSON'].sum(),'child':home_conflict_df[home_conflict_df.move_type==2]['CHILD'].sum(),
#                 'male':home_conflict_df[home_conflict_df.move_type==2]['ADULT_MALE'].sum(),'female':home_conflict_df[home_conflict_df.move_type==2]['ADULT_FEMALE'].sum()}
#     #print(new_row)
#     simulated_refugee_df = simulated_refugee_df.append(new_row,ignore_index=True)
    
#     home_conflict_df['move_date'] = str(min_date)
#     hid_displacement_df.append(home_conflict_df[home_conflict_df.move_type!=0])
#     home_conflict_df = home_conflict_df.drop(columns=['another_random','move_type','move_date'])
