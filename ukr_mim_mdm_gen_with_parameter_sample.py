...
D = random.uniform(2.0,2.2)#2.212#random.uniform(1.0,10.0) #1.5/2.0 ## kind of tuned
A = random.uniform(54.0,56.0)#55.0#random.uniform(50.0,100.0) #70.0 ## calibrated
T = random.uniform(0.7,0.8)#0.253##random.uniform(0.5, 3.0) #0.8/1.0 ## calibrated
...
ews = random.uniform(0.32,0.36)#0.34#random.uniform(0.2,2.0)
pactive = 24#random.randint(15,30)
peer_thresh_hi = 2#random.randint(0,1)
use_neighbor = 5#random.sample([2,5,8,10],1)[0]#random.unifrom(0.0,1.0)
border_cross_prob = random.uniform(0.25,0.35)
...
for name in all_ids:
    print('sbatch ukr_mim_mdm_sample_no_idp.sbatch',name,hyper_comb,D,A,T,S,t_l,t_r,ps,ews,pactive,peer_thresh_hi,use_neighbor,border_cross_prob)
...
df = pd.DataFrame(columns=['hyper_comb','DISTANCE_DECAY','SIGMOID_SCALAR','SIGMOID_EXPONENT','MEMORY_DECAY',
                           'TIME_LEFT','TIME_RIGHT','BIAS_SCALE','EVENT_WEIGHT_SCALE','PEER_AFFECT_ACTIVE_DAY','THRESH_HI','USE_NEIGHBOR','BORDER_CROSS_PROB'])
...
new_row = {'hyper_comb':hyper_comb,'DISTANCE_DECAY':D,'SIGMOID_SCALAR':A,'SIGMOID_EXPONENT':T,
           ...
           'BIAS_SCALE':ps,'EVENT_WEIGHT_SCALE':ews,'PEER_AFFECT_ACTIVE_DAY':pactive,'THRESH_HI':peer_thresh_hi,
           'USE_NEIGHBOR':use_neighbor,'BORDER_CROSS_PROB':border_cross_prob}

df = df.append(new_row,ignore_index=True)

df.to_csv('runtime_log/parameter_comb.csv', mode='a', index=False, header=False)
